...
/**
 * Apply all the configuration related to the OAuth2 "handshake" with Shopify, as defined in
...
public class ShopifyOAuth2 implements HttpSecurityBuilderConfigurerDelegate {
  ...
  /**
   * Configure the OAuth2LoginConfigurer. Set the custom OAuth2AuthorizationRequestResolver, the
   * base uri on the Redirection Endpoint, a custom OAuth2AccessTokenResponseClient on the Token
   * Endpoint, a custom OAuth2UserService on the UserInfo Endpoint, an AuthenticationSuccessHandler,
   * the login page, and the failure uri. The objects are obtained from ShopifyBeansUtils.
   *
  ...
  public void applyShopifyConfig(HttpSecurityBuilder<?> http) {
    ...
    OAuth2LoginConfigurer<HttpSecurity> configurer = http
        .getConfigurer(OAuth2LoginConfigurer.class);
    ...
    if (configurer != null) {
      configurer.authorizationEndpoint()
          ...
          .and()
          .redirectionEndpoint()
          .baseUri(this.anyAuthorizationRedirectPath) // same as filterProcessesUrl
          .and()
          .tokenEndpoint().accessTokenResponseClient(
          getAccessTokenResponseClient()) // allows for seamless unit testing
          .and()
          .userInfoEndpoint().userService(getUserService())
          .and()
          .successHandler(getSuccessHandler())
          .loginPage(
              this.loginEndpoint) // for use outside of an embedded app since it involves a redirect
          .failureUrl(
              this.authenticationFailureUrl); // see AbstractAuthenticationFilterConfigurer and AbstractAuthenticationProcessingFilter
    }
  ...
  private ClientRegistrationRepository clientRegistrationRepository(
      ClientRegistration shopifyClientRegistration) {
    return new InMemoryClientRegistrationRepository(shopifyClientRegistration);
  }

  private ShopifyHttpSessionOAuth2AuthorizationRequestRepository customAuthorizationRequestRepository(
